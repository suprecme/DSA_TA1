...
class Admin extends User {
    ...
    public int tambahLagu(Lagu[] playlist, int jumlahLagu, Scanner input) {
        ...
        if (jumlahLagu < playlist.length) {
            ...
            // Input durasi dibaca sebagai String agar bisa menerima koma atau titik
            System.out.print("Masukkan durasi lagu (menit): ");
            String durasiInput = input.nextLine().replace(',', '.');

            try {
                // Mengubah input durasi dari String menjadi double
                double durasi = Double.parseDouble(durasiInput);

                // Validasi agar durasi harus lebih dari 0 menit
                if (durasi > 0) {
                    ...
                    System.out.println("Durasi tidak valid. Durasi harus lebih dari 0 menit.");
                }

            } catch (NumberFormatException e) {
                // Menampilkan pesan jika input durasi bukan angka
                System.out.println("Input durasi tidak valid. Masukkan angka, contoh 3.45 atau 3,45.");
            }
